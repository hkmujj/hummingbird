...
def __init__():
	...
	result_dict = {}
	filename = os.path.join(os.path.dirname(os.path.abspath(__file__)), os.sep.join(['package_sql','数据字典20120926.xls']))
	...
	sub_r.append(9999)
	# 主题域所占行数
	result_dict['sub_list_index'] = dict(zip(result_dict['sub_list'],zip(sub_l, sub_r))) # 每个域的开始和结束行
	result_dict['tbl_names_c'] = s1.col_values(1)[0:]
	result_dict['tbl_names'] = s1.col_values(2)[0:]
	col_name = s1.col_values(3)  # 字段英文名
	col_name_c = s1.col_values(4) # 字段中文名
	result_dict['tbl_list'] = reduce(lambda a,b: b in a and a or a+[b], [[]]+['%s' % i for i in result_dict['tbl_names'] if i])
	sub_l = map(lambda x: result_dict['tbl_names'].index(x), result_dict['tbl_list'])
	sub_r = sub_l[1:]
	sub_r.append(9999)
	# 每个表所占行的起始
	result_dict['tbl_list_index'] = dict(zip(result_dict['tbl_list'],zip(sub_l, sub_r))) # 的开始和结束行
	dict_tbl = {}
	for tbl in result_dict['tbl_list']:
		#print tbl
		start = result_dict['tbl_list_index'][tbl]
		#print start
		#print col_name.__getslice__(*start)
		dict_tbl[tbl] = dict(zip(col_name.__getslice__(*start), col_name_c.__getslice__(*start)))
	result_dict['col_dict'] = dict_tbl
...
def dis_sub(*k, **kw):
	...
	for idx, col in enumerate(tmp_k0):
		if col<>tmp_k0[idx-1]: # 如果本行与上一行不同,则证明是一个新的表出现,就给出一个到该表的链接.
...
def dis_table(*k ,**kw):
	...
	tbl = table_name = tbl_names[index]
	#yield 'tbl_name'
	#yield table_name.encode('utf-8')
	t_name = [table_name]
	if k[0].decode('utf-8') in add_area:
		if t_name[0].find('AREA')<>-1:
			t_name[0] = t_name[0].replace(u'AREA',u'AREA_'+fengongshi)
			table_name = table_name.replace(u'AREA',u'AREA_'+fengongshi)
	...
	if table_name.endswith('YYYYMMDD') or tbl_names_c[index].find(u'日表')<>-1:
		...
		t_name.append(int((datetime.datetime.now()+datetime.timedelta(days=-8)).strftime('%Y%m')))
		if t_name[-1]%100<>1:
			t_name[-1] -= 1 
		else:	t_name[-1] -= 89
	...
	try:
		...
		return
	yield '<form>'
	yield '%s' % INPUT(map(lambda x: x+' '+col_dict[tbl].get(x.upper(),x).encode('utf-8', 'ignore'), recs._columns),name=['col_name']*len(recs._columns), value=recs._columns, type="radio", checked=kw_n['col_name'])
	print 'tbl:', col_dict[tbl]
	yield '<br />'
	...
	yield '<table border=2 cellspace=2>'
	yield '%s' % TR(TD(map(lambda x: x+'<br />'+col_dict[tbl].get(x.upper(),x).encode('utf-8', 'ignore'), recs._columns)))
	for rec in recs:
