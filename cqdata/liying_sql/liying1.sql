...
drop table if exists tmp_tgm_1;
create temp table tmp_tgm_1 as 
...
select  count(*), sum(charge) from tmp_tgm_1;

drop table if exists tmp_tgm_t2;
create temp table tmp_tgm_t2 as 
select a.*, case when b.completed_date like '{yyyy-mm}%%' then 1 else 0 end new_flag
--, b.product_id 
, product_name
,mkt.mkt_channel_name
from tmp_tgm_1 a left join ds_chn_prd_serv_{area}_{yyyymm} b on a.serv_id=b.serv_id
