...
--执行开始
--PPT3
--四级套餐类别和个数，我的E家类，如果要改其他，请改下面的offerd='XXXXXXX套餐'，如果需要再进一步分析六级套餐，请将OFFER_COMP_TYPE_DESC4改为OFFER_COMP_TYPE_DESC6
-- drop table if exists cstt5;
create temporary table cstt5 as
select distinct OFFER_COMP_TYPE_DESC4 四级套餐类别,count(distinct offer_comp_instance_id) 商品包个数 
from 
csttl 
where 
offerd='我的e家品牌套餐销售品'
...
select distinct a.OFFER_COMP_TYPE_DESC4 四级套餐类别,sum(amount) 商品包下挂设备费用
from 
csttl a,DS_ACT_ACCT_ITEM_FEEMONTH b
where 
a.serv_id=b.serv_id
and 
a.offerd='我的e家品牌套餐销售品'
group by 1;
--四级套餐ARPU值
--drop table if exists tmp_PACKGE_NAME_AREA_FEEMONTH;
create table tmp_PACKGE_NAME_AREA_FEEMONTH as 
select distinct a.四级套餐类别,a.商品包个数,b.商品包下挂设备费用,b.商品包下挂设备费用/a.商品包个数 arpu
from 
cstt5 a,cstt6 b
where 
a.四级套餐类别=b.四级套餐类别;
