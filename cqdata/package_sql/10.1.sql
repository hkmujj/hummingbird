...
select distinct OFFER_COMP_TYPE_DESC4 套餐四级名称,BALANCE 使用余额,peak_value 额定总值,BALANCE/peak_value 套餐使用率 

from 
(
select distinct d.OFFER_COMP_TYPE_DESC4,sum(b.BALANCE) BALANCE,sum(b.peak_value) peak_value          --如需要号码对应套餐、套餐使用值清单，则在此行增加a.acc_nbr,e.offer_name,f.PRICING_PLAN_NAME等信息
from 
...
--(values (select date_trunc('month', '2011-08-01 00:00:00'    --此处时间是销售品实例表的方案失效时间，大于统计账期的次月零点
and 
...
and PRICING_PLAN_NAME not like '%%条%%' 
and PRICING_PLAN_NAME not like '%%流量%%' 
and PRICING_PLAN_NAME not like '%%彩信%%' 
and PRICING_PLAN_NAME not like '%%短信%%' 
and PRICING_PLAN_NAME not like '%%上网%%' 
and PRICING_PLAN_NAME not like '%%1X%%' 
and PRICING_PLAN_NAME not like '%%T2%%' 
and PRICING_PLAN_NAME not like '%%小时%%' 
and PRICING_PLAN_NAME not like '%%免费%%' 
group by 1
...
(
select distinct d.OFFER_COMP_TYPE_DESC3,sum(b.BALANCE) BALANCE,sum(b.peak_value) 
peak_value
from 
DS_CHN_PRD_SERV_AREA_FEEMONTH a,  --用户月表请替换为对应分公司
DM_PRD_OFFER_ACCUMULATOR b,   --套餐余额表，目前只有上月月数据，以后会增加日表和月表，同时对应分公司
