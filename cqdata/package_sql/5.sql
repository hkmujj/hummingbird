...
--drop table if exists if exists tmp_PACKGE_NAME_AREA_FEEMONTH;
...
select distinct b.OFFER_COMP_TYPE_DESC6,count(distinct c.offer_comp_instance_id) oid
from 
tmp_jg_AREA_FEEMONTH a,DIM_WD_OFFER_NEW_DIR_SECOND b,DS_CHN_PRD_OFFER_COMP_FEEMONTH c,DS_CHN_PRD_CUST_30_FEEMONTH d, --套餐月表，改账期
...
a.completed_date<(Values ((SELECT date_trunc('month', TIMESTAMP 'FEEMONTH01'))))
and 
a.mkt_cust_code=d.mkt_cust_code 
and 
d.CUST_GRP_TYPE=10 --政企标识
and 
c.comp_offer_id=e.offer_id
and 
e.offer_kind='C'
group by 1
