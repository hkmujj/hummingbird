...
import os, sqlite3, MySQL, threading, my_tools
...
class ami(MySQL.MyTable):
	...
	def __init__(self, *k, **kw):
		...
		#self.__dict__['_conn'].text_factory = str #lambda x:my_tools.auto_decode(x, 'utf-8')
		...
		if class_count[threading.currentThread().getName()] == 0:
			if self.__dict__['_conn'].total_changes > 0:	# 有改变动生,正处于一个事务中
				if ami._error: 
					...
					#print '执行事务%s...' % ami._conn.total_changes
					print u'执行事务%s...' % self.__dict__['_conn'].total_changes
