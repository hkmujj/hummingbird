...
import sqlite3, os, chardet, MySQL, cherrypy, cgi, sys, dbapi, my_tools, chardet, db_files, traceback, time
exposed = True
name = '老城月光'
...
def message(*k):
	...
	rec = dbapi.event(k)[0]
	rec.__dict__['content'] = cgi.escape(rec.__dict__['content']).replace(' ','&nbsp;').replace('\n','<br />')
	yield ''.join(header('..'))
	try:
...
def _new(**kw):
	...
	if 'zfile' in kw:
		print(dir(kw['zfile']))
		zf = kw['zfile']
		fn = kw['zfile'].filename
		#code = chardet.detect(fn)
		s = zf.read()
		print len(s), 'len...'
		print zf.file
		print zf.charset
		print kw['zfile'].length, 'LEN'
		print zf.name
		print fn.decode('utf-8', 'ignore')
	if not kw.get('title',''):
		return '题目不能为空'

	rec = dbapi.event()._insert()
	rec.title = kw['title'].encode('gb2312', 'ignore')
	title = kw['title']
	try:
		print('TRY detect code')
		code = chardet.detect(kw['content'])
		print('CODE:', code)
		if code['encoding']=='utf-8':
			content = kw['content'].decode('utf-8','ignore')
		else:
			content = kw['content']
		rec.content = content.encode('gb2312', 'ignore')
		#rec.content = my_tools.auto_decode(kw['content'],'gb2312')
	except:
		traceback.print_exc()
		raise
	rec.id_up = 0
	rec.id_root = rec.id
	rec.id_up_s = ''
	rec.time_create = rec.time_access = rec.time_modify = rec.time_last_follow = time.strftime('%Y-%m-%d %H:%M:%S')
	print 'update search...'
	search = my_tools.fenchi(title, content, rec.user, rec.state)
	print search
	dbapi.search(rec.id)._insert().keyword = search.encode('gb2312','ignore')
	del rec
	return ''
...
def edit(*k,**kw):
	...
	if kw.get('submit','') == u'修改':
		...
		if title<>rec.title:
			...
			rec.title = kw['title'].encode('gb2312', 'ignore')
			update_f = 1
		print 'KW content', kw['content']
		content = kw['content']
		content = my_tools.auto_decode(kw['content'], 'utf-8')
		#print 'KW content', content
		#print(chardet.detect(kw['content']))
		if content<>rec.content:
			print 'update con'
			#print 'CONTENT:', my_tools.auto_decode(kw['content'],'utf-8')
			rec.content = content.decode('utf-8').encode('gb2312', 'ignore')
			update_f = 1
		if update_f: # 作了更新,因此要同步更新search表
			print 'update search...'
			search = my_tools.fenchi(title, content, rec.user, rec.state)
			print search
			search_rec = dbapi.search(int(k[0]))
			if len(search_rec):
				dbapi.search(int(k[0])).keyword = search.encode('gb2312','ignore')
			else:
				search_rec._insert(keyword=search.encode('gb2312','ignore'))
	yield ''.join(header('..'))
